...
describe("Utility functions", () => {
  ...
  test("registerFormSchema validates postal code format", () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('Le code postal doit être au format français (5 chiffres)')
    }
  })
  ...
  test("registerFormSchema validates name format (no special characters)", () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].message).toBe('Le prénom ne doit contenir que des lettres, espaces, et tirets')
    }
  })
